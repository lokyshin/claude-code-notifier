...
### Hook 子进程 tmux 环境变量丢失

**问题**：Claude Code 的 Hook 以子进程方式执行 `notify.sh`，该子进程可能不继承 `$TMUX` 和 `$TMUX_PANE` 环境变量，导致日志中出现：

```
remote-approve | no TTY and no tmux pane found, skip polling
```

**解决方案**：在 `~/.claude/notifier.conf` 中添加动态检测逻辑：
...
此配置可直接复制到任何新服务器，无需修改。逻辑：

1. 若 `$TMUX` 已存在（正常情况）→ 直接用
2. 若不存在 → 自动找当前用户的 tmux socket 并重建
3. 若 `$TARGET_PANE` 和 `$TMUX_PANE` 都没有 → 自动找正在跑 claude/node/bun 的 pane
...
当 Claude Code 运行在无法被手机直接访问的内网/云服务器（如 AWS）时，可以将审批服务单独部署到一台有公网访问能力的服务器上。

**架构**：

```
┌─ 内网服务器（Claude Code）─┐       ┌─ 公网服务器（审批服务）─┐       ┌─ 手机 ─┐
│                             │       │                         │       │        │
│ notify.sh ──出站POST─────→  │ ───→  │ Flask:9120              │ ←──── │ 飞书   │
│            ──出站轮询─────→  │ ───→  │ (审批页面)              │ ───→  │ 审批   │
│ tmux inject ← 拿到结果      │       │                         │       │        │
└─────────────────────────────┘       └─────────────────────────┘       └────────┘
```
...
- 内网服务器只需出站网络，无需入站端口
- 审批页面在公网服务器，手机直接访问
- 多台 Claude Code 实例可共享同一个审批服务（通过 hostname 区分来源）
...
USE_REMOTE_APPROVE=1
APPROVE_SERVER="https://你的公网服务器域名"   # 指向远程审批服务
APPROVE_INTERVAL=3
APPROVE_EXPIRE=300
```

**公网服务器**：只需部署 `approve-server/` 并配好 Nginx + HTTPS。
...
| `TARGET_PANE` | (自动检测) | 强制指定 tmux pane ID（如 `%1`），用于 hook 子进程无法自动检测的情况 |
